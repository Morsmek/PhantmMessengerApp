package com.phantm.messaging.data.crypto
...
object Bip39Helper {
    ...
    private val wordSet = WORD_LIST.toSet()

    fun generateMnemonic(wordCount: Int = 20): String {
        val selected = mutableListOf<String>()
        for (i in 0 until wordCount) {
            selected.add(WORD_LIST[Random.nextInt(WORD_LIST.size)])
        }
        return selected.joinToString(" ")
    }

    fun isValidWord(word: String): Boolean {
        return wordSet.contains(word.trim().lowercase())
    }

    fun generateKeyPair(mnemonic: String): String {
        val digest = MessageDigest.getInstance("SHA-256")
        val hashBytes = digest.digest(mnemonic.trim().lowercase().toByteArray(Charsets.UTF_8))
